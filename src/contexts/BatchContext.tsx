...
interface BatchContextType {
  ...
  /** `directorIds: []` explicitly unassigns all Batch Directors — distinct from omitting the key, which leaves it untouched. */
...
export function BatchProvider({ children }: { children: React.ReactNode }) {
  ...
  const getBatchesByDirector = useCallback(
    (directorId: string) => batches.filter((b) => b.directorIds?.includes(directorId)),
    [batches],
  );
